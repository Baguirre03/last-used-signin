...
class PopupManager {
    ...
    renderCurrentSite() {
        ...
        const currentSite = this.allSites.find((site) => site.domain === this.currentDomain);
        if (providerElement) {
            if (currentSite) {
    ...
    renderSitesList() {
        ...
        const filteredSites = this.allSites.filter((site) => site.domain.toLowerCase().includes(searchTerm) ||
        ...
        <div class="site-item">
          ...
          <div class="last-used">
            ...
            <button class="clear-btn" data-domain="${this.escapeHtml(site.domain)}" title="Clear for ${this.escapeHtml(site.domain)}">✕</button>
          </div>
        </div>
      `)
            .join("");
